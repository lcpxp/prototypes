...
test("a Daopay user gets none of the Acquirer-only controls", () => {
...
test("a Acquirer user keeps every control except the two Daopay sends", () => {
...
test("ACQUIRER is the scoping key and it narrows the list", () => {
  ...
  for (const row of all) {
    assert.ok(["Acquirer", "DaoPay", "-"].includes(row.acquirer),
...
test("generate is a Acquirer control, send is not", () => {
  // The whole point of the empty tables: Acquirer generates, Daopay sends.
  const pxp = load(AS_PXP);
  const daopay = load(AS_DAOPAY);
  assert.equal(pxp.can("generateContract"), true);
  assert.equal(pxp.can("generateKyc"), true);
  assert.equal(pxp.can("sendContract"), false);
  assert.equal(daopay.can("generateContract"), false);
  assert.equal(daopay.can("sendContract"), true);
});
