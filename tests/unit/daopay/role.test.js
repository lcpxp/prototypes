...
test("the contract tables begin empty, generated by the PXP user", () => {
  // The tables start with no rows; a PXP user's Generate contract adds
...
test("generate is a PXP control, send is not", () => {
  // The whole point of the empty tables: PXP generates, Daopay sends.
  const pxp = load(AS_PXP);
  const daopay = load(AS_DAOPAY);
