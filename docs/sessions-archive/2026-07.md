...
- Reference split into distinct sites: api_specs gains a family
  ...
  shells: "LP API" and "Merchant Portal API".
...
3. Fill the two live spec shells with endpoint rows (LP
   inbound flows first, then Merchant Portal endpoints, then
   Merchant Portal-initiated repurchase flows as tagged sections).
...
Branch: claude/lcps-setup-structure-19n2mo
Completed:
...
Branch: main
Completed:
- Reframed the experience around LPIO as a top-level
