...
- API reference coverage to collect from the existing codebase and
  developers: inbound API, self-service API, and outgoing
  integrations (EIT, Hubspot, Dynamics, Daopay, KPI portal, Unity
  portal, payment service, screening checks).
- Dashboards and insights: overview and ordering.
- An LLM-backed ask feature over the hub's own material.
- Hosting of screen recordings, videos and key reference material.
- A structured data repository for future plans, PRDs and intents.
- Automated AI meeting notes, kept in condensed and granular records.
