-- ------------------------------------------------------------------
-- 52_portal_review.sql - Portal review: walking the portal area by
-- area, recording what is wrong, what was answered, what was verified,
-- and what the review decided (see docs/PORTAL-REVIEW.md).
--
-- Sibling of application review (50_review.sql) and deliberately the
-- same architecture: waves as objects, concepts never collapsed,
-- revisions written by trigger, read-only in the browser, every write
-- from a Claude session. review_waves is SHARED - a wave carries a
-- `kind`, so opening, carrying forward and closing work identically
-- whichever sort of review it is.
--
...
-- A finding can be closed AND promoted at once, which is why the first
-- two are separate questions. verified_at is never set by a session,
-- for the same reason review_applications.confirmed_at is not: a
-- developer saying "done" is a claim, and the reviewer verifying it is
-- a decision.
--
-- Nothing derivable is stored: coverage percentages, per-area open
-- counts, the three-way grouping and the walker's queue are all
