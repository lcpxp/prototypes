-- ------------------------------------------------------------------
-- Applied 2026-08-09 as knowledge_links_typed_vocabulary.
-- Creates the typed link graph that replaces work_items.relates_to_id:
-- link_entity_types, link_kinds (eight SKOS-shaped kinds),
-- knowledge_links (polymorphic, bi-temporal), the two guard triggers,
-- the knowledge_graph and knowledge_links_orphans views, and the RLS
-- policies mirroring supabase/policies.sql. Also drops the empty
-- work_item_dependencies table, whose one relationship type is now the
-- 'blocks' kind.
--
-- Full rationale beside each object: supabase/schema/33_links.sql.
...
-- work_item_dependencies folds into the 'blocks' kind. It was empty.
drop table if exists public.work_item_dependencies;

-- RLS mirroring supabase/policies.sql, which stays the authoritative
-- home for these policies (see its header).
...
begin
  ...
  loop
    ...
    execute format('create policy "%s: members read" on public.%I for select to authenticated using (%s)', entry.tbl, entry.tbl, entry.read_expr);
    execute format('create policy "%s: admins insert" on public.%I for insert to authenticated with check ((select public.is_admin()))', entry.tbl, entry.tbl);
    execute format('create policy "%s: admins update" on public.%I for update to authenticated using ((select public.is_admin())) with check ((select public.is_admin()))', entry.tbl, entry.tbl);
    execute format('create policy "%s: admins delete" on public.%I for delete to authenticated using ((select public.is_admin()))', entry.tbl, entry.tbl);
  end loop;
end $pol$;
