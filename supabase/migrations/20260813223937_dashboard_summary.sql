-- ------------------------------------------------------------------
-- The dashboard rebuild (docs/plan/50-DASHBOARD.md). Additive only:
-- one nullable column and one new function. dashboard_counts() stays
-- until nothing calls it.
-- ------------------------------------------------------------------

-- One sentence saying what a tool does and when to use it. The nav
-- shows icons with tooltips and explains nothing; the dashboard's
-- tools grid needs prose, and prose about an internal tool belongs
-- with the row, not in a public repo.
alter table public.portal_links
  add column if not exists description text;

-- dashboard_summary: everything the landing page needs, in one round
-- trip. SECURITY INVOKER, so RLS filters every figure to what the
-- caller may read - a member with no roadmap grant gets an empty
-- workstream list rather than an error. Every subquery is capped or
-- bounded; nothing reads a heavy jsonb column.
...
as $$
  select jsonb_build_object(
    ...
    'reviews', (
      select coalesce(jsonb_agg(jsonb_build_object(
        'id', w.id, 'name', w.name, 'state', w.state, 'opened_at', w.opened_at,
        'applications', (select count(*) from public.review_applications a
                         where a.wave_id = w.id and a.deleted_at is null),
        'needs_action', (select count(*) from public.review_applications a
                         join public.triage_categories t on t.key = a.triage_category
                         where a.wave_id = w.id and a.deleted_at is null
                           and a.confirmed_at is null and t.group_key = 'needs_action'),
        'unconfirmed',  (select count(*) from public.review_applications a
                         where a.wave_id = w.id and a.deleted_at is null
