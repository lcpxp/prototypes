-- ------------------------------------------------------------------
-- The semantic channel, blended into roadmap_find.
--
-- Two shape changes: p_embedding (optional - passed null, the function
-- scores exactly as it did before), and two extra returned columns,
-- lexical and semantic, so a reader can see which channel spoke.
--
-- The reasoning and the fitted rescale live in
-- supabase/schema/31_roadmap_search.sql, where they can be kept current.
-- Applied migrations are immutable, so this file is the record that the
-- change was applied, carrying the body verbatim.
-- ------------------------------------------------------------------
