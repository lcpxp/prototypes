...
(function () {
  ...
  App.pciInterstitial = {
    open: function (ctx) {
      var root = document.getElementById("ps-modal-root");
      var ixopay = App.pciIxopay;

      function close() { root.innerHTML = ""; }
      function finish() { close(); if (ctx.onDone) ctx.onDone(); }
      function shell(body) {
        root.innerHTML = '<div class="ps-modal-scrim"><div class="ps-inter" role="dialog" aria-label="PCI compliance">' + body + "</div></div>";
      }
      function q(sel) { return root.querySelector(sel); }
      function on(sel, fn) { var e = q(sel); if (e) e.addEventListener("click", fn); }

      function head(title, sub) {
        return '<div class="ps-inter-head"><span class="ps-inter-eyebrow">PCI DSS compliance</span>' +
          "<h2>" + esc(title) + "</h2><p>" + esc(sub) + "</p></div>";
      }
      function foot(skipLabel) {
        return '<div class="ps-inter-foot"><button type="button" class="ps-inter-skip" data-skip>' +
      ...
      function ask() {
        shell(
          ...
          '<div class="ps-inter-body"><div class="ps-inter-options">' +
          '<button type="button" class="ps-inter-card" data-yes><h3>Yes, already compliant</h3>' +
          "<p>Record their status. We can verify it with IXOPAY and, if you like, keep it monitored.</p></button>" +
          '<button type="button" class="ps-inter-card" data-no><h3>No, or not sure</h3>' +
      ...
      function compliantPath() {
        shell(
          ...
          '<div class="ps-inter-body">' +
          '<button type="button" class="ps-btn ps-btn--ghost" data-check>Check status with IXOPAY</button>' +
          '<div id="ps-check-result"></div>' +
          '<div class="ps-engage"><div><strong>Support ongoing compliance?</strong>' +
          "<p class=\"ps-hint\">We can run their annual checks and scans and chase renewals so it never lapses.</p></div></div>" +
          '<div class="ps-inter-options"><button type="button" class="ps-inter-card" data-engage><h3>Yes, enrol for managed compliance</h3>' +
          "<p>We handle the annual cycle. A PCI Compliance Fee is added to the quote.</p></button>" +
          '<button type="button" class="ps-inter-card" data-justcompliant><h3>No thanks, continue</h3>' +
        ...
        on("[data-check]", function () {
          ...
          var host = document.getElementById("ps-check-result");
          host.innerHTML = res.found
            ? '<div class="ps-note ps-note--ok ps-mt16">Found on IXOPAY records: ' + esc(res.status) + ", valid until " + esc(res.expiryDate) + ".</div>"
            : '<div class="ps-note ps-note--info ps-mt16">No record found. You can still enrol them below.</div>';
        });
      ...
      function enrolReview() {
        ...
        shell(
          ...
          '<div class="ps-inter-body"><div class="ps-review"><dl>' +
          ...
          '<div class="ps-actions"><button type="button" class="ps-btn ps-btn--primary" data-continue>Continue to enrol</button></div>' +
      ...
      function enrolEmail() {
        shell(
          ...
          '<div class="ps-inter-body"><div class="ps-field"><label>Merchant email for the invitation</label>' +
          '<input class="ps-input" id="ps-enrol-email" type="email" value="' + esc(ctx.merchant.email) + '"></div>' +
          '<div class="ps-actions"><button type="button" class="ps-btn ps-btn--primary" data-send>Send invitation</button></div>' +
          "</div>" + foot("Skip for now")
        );
        on("[data-send]", function () {
          var email = (document.getElementById("ps-enrol-email") || {}).value || ctx.merchant.email;
      ...
      function sent(email) {
        shell(
          ...
          '<div class="ps-inter-body"><div class="ps-note ps-note--ok">Invitation sent to ' + esc(email) +
          ...
          '<div class="ps-actions"><button type="button" class="ps-btn ps-btn--primary" data-done>Back to Products &amp; Pricing</button></div></div>'
        );
        on("[data-done]", finish);
      }
